...
interface TrainingExercise {
  ...
  created_at: string
  tussenOefeningen?: TrainingExercise[]
}
...
const TrainingScheduleDetail = ({ user }: TrainingScheduleDetailProps) => {
  ...
  const loadScheduleData = async () => {
    try {
      ...
      // Load exercises - alleen basis oefeningen, met hun tussen oefeningen
      ...
      if (exercisesError) {
        ...
        // Groepeer exercises: basis exercises met hun tussen oefeningen
        const basisExercises = (allExercises || []).filter(ex => ex.exercise_type === 'basis' && !ex.parent_exercise_id)
        const exercisesWithTussen = basisExercises.map(basis => ({
          ...basis,
          tussenOefeningen: (allExercises || []).filter(ex => ex.parent_exercise_id === basis.id)
        }))
        setExercises(exercisesWithTussen)
      }
    } catch (err) {
